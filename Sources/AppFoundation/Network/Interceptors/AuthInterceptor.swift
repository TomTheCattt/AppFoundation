...
public final class AuthInterceptor: RequestInterceptor, @unchecked Sendable {
    ...
    private let refreshAction: (@Sendable () async throws -> Bool)?
    private let lock = NSLock()
    private var _isRefreshing = false
    
    private var isRefreshing: Bool {
        get { lock.withLock { _isRefreshing } }
        set { lock.withLock { _isRefreshing = newValue } }
    }
    
    init(tokenStore: TokenStoreProtocol, logger: Logger, refreshAction: (@Sendable () async throws -> Bool)? = nil) {
